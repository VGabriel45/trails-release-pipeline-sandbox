...
function isPublishedOnNpm(
  pkg,
  {
    intervalMs = NPM_VIEW_POLL_INTERVAL_MS,
    timeoutMs = NPM_VIEW_POLL_TIMEOUT_MS,
  } = {},
) {
  ...
  for (const pkg of pkgs) {
    ...
    if (notes) {
      const notesFile = join(
        tmpdir(),
        `release-notes-${pkg.name.replace(/[^\w.-]+/g, "-")}-${pkg.version}.md`,
      )
...
function changelogSection(pkgName, version) {
  const entry = getPublishablePackageEntries().find((p) => p.name === pkgName)
  if (!entry) return null

  let md
  try {
    md = readFileSync(join(entry.dir, "CHANGELOG.md"), "utf8")
  } catch {
    return null
  }
...
const releaseCandidates = packagesForCurrentReleaseCommit(
  allPublishable,
  expectedCommit,
)
...
if (publishFailed) {
  console.error(
    "Publish finished with npm errors (tags/releases synced for published versions only).",
  )
  process.exit(1)
}
